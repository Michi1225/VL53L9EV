...
int platform_delay_ms(uint32_t delay_ms) {
...
int platform_get_version(platform_version_t *version) {
    ...
    strncpy(version->board_name, "nucleo-n657x0-q", BOARD_NAME_STR_SIZE);
...
int platform_enable_event(platform_event_t event) {
    ...
    case PLATFORM_I3C_IBI_EVT:
        ...
        HAL_I3C_ActivateNotification(&hi3c2, NULL, LL_I3C_IER_IBIIE);
...
int platform_disable_event(platform_event_t event) {
    ...
    switch (event) {
    case PLATFORM_I3C_IBI_EVT:
        HAL_I3C_DeactivateNotification(&hi3c2, LL_I3C_IER_IBIIE);
