...
UINT MX_USBX_Device_Init(VOID *memory_ptr)
{
  ...
  if (tx_event_flags_create(&usb_device_events,
                            "USB Device Events") != TX_SUCCESS)
  {
  ...
  if (tx_thread_create(&ux_device_app_thread, UX_DEVICE_APP_THREAD_NAME, usbx_app_thread_entry,
...
UINT MX_USBX_Device_Stack_Init(void)
{
  ...
  ux_dcd_stm32_initialize((ULONG)USB2_OTG_HS, (ULONG)&hpcd_USB_OTG_HS2);
...
UINT MX_USBX_Device_Stack_DeInit(void)
{
  ...
  if (_ux_dcd_stm32_uninitialize((ULONG)USB2_OTG_HS, (ULONG)&hpcd_USB_OTG_HS2) != UX_SUCCESS)
  {
    return UX_ERROR;
  }
